...
double thresholdv = 0.08;  //V , 126mV should be the max value of plant sound 
...
int main(int argc, char *argv[])
{
    ...
    while(1){
        ...
        if (triggered && buffer_index == index_end) {
            ...
            if (lower_freq<freqmaxmain && freqmaxmain<upper_freq){
                trignb++;
                // if (trignb>160){
                ...
                //     bcm2835_spi_end();
                //     bcm2835_close();
                //     printf("max file nb reached everything is closed\n");   
                //     exit(0);

                // }
                printf("\nPlant sound detected nb %d\n", trignb);
                //csvsave(bufferc, trignb, threshold);
            }
